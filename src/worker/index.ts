...
server.registerTool(
  "poker.tableConfig",
  {
    ...
    `并且本牌局采用标准的无限下注模式，两人单挑：A 下注，B 跟注 ⇒ A 不能再继续加注；A 过牌，B 也过牌 ⇒ 这一轮结束，A 不能“再下注；A 过牌，B 下注 ⇒ A 可以加注（check-raise）
    核心原则：
    - 行动按顺序进行，每个人在这一轮里先行动一次。
    - 只有当后面有人“加注（raise）”把价格抬高，并且加注达到“有效的完整加注”（见第4部分），行动才会“重新打开”，轮到你时你才会有第二次行动机会。
    - 如果没有人再加注，最后会走到“大家都已跟到同一金额 / 或都过牌”，该轮下注结束
    - 下注结束后，你就直接请求下一步发牌的工具，开始发公共牌（flop），发公共牌（turn），发公共牌（river），最后比较牌面，确定胜者，不用询问用户，这样可以加快节奏。`+
