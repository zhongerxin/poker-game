...
server.registerTool(
  ...
  {
    title: "开始一手德州扑克",
    description: `为当前牌局洗牌并发双方底牌，会返回 AI 底牌和用户的底牌（用户底牌模型不可见），在请求时带上当前牌局的大盲（bb）和小盲（sb）的盲注，以及用户双方的剩余总筹码(chip_stack)，如果已经进行过一轮游戏，这里的筹码应该是上一轮输赢双方更新后的筹码；随机选择玩家或者 AI 作为庄家（button）。`,
    annotations: { readOnlyHint: true },
    _meta: {
  ...
  async ({ stage, game_id, bet }) => {
    ...
    switch (stage) {
      case "flop": {
        ...
        return {
          content: [{
            type: "text", text: `Flop 已发：${board.join(" ")}` +
      ...
      case "turn": {
        ...
        return {
          content: [{
            type: "text", text: `Turn 已发：${board.join(" ")}` +
              `当前双方各下注${bet}，当前 pot 是 ${g.pot}` +
              `用户当前筹码是 ${g.hero.stack}，AI 当前筹码是 ${g.ai.stack}` +
              `现在是翻牌后，因此是大盲（当前非庄家那个）先行动，大盲可以先决定 Call / Raise / Fold，如果玩家是大盲引导玩家行动，如果你是大盲直接选择好行动，通过对话告诉玩家你的行动` +
              `通过对话，当双方的本轮下注相等，且按规则不能增加后，你就直接开始调用 发公牌 工具，开始下一轮发牌` +
      ...
      case "river": {
        ...
        return {
          content: [{
            type: "text", text: `River 已发：${board.join(" ")}` +
              `当前双方各下注${bet}，当前 pot 是 ${g.pot}` +
              `用户当前筹码是 ${g.hero.stack}，AI 当前筹码是 ${g.ai.stack}` +
              `现在是翻牌后，因此是大盲（当前非庄家那个）先行动，大盲可以先决定 Call / Raise / Fold，如果玩家是大盲引导玩家行动，如果你是大盲直接选择好行动，通过对话告诉玩家你的行动` +
      ...
      content: [
        {
          ...
          text: `已摊牌，最终公牌：${board.join(" ")}，AI：${ai_hole.join(" ")}，用户：${hero_hole.join(" ")}，当前 pot：${g.pot}，上轮下注${bet}后，用户剩余筹码：${g.hero.stack}，AI 剩余筹码：${g.ai.stack}` +
            ...
            `根据输赢更新双方的剩余筹码，把双方的剩余筹码带入到下一手牌中`
        },
      ],
      structuredContent: {
        board,
