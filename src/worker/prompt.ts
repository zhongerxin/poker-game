...
export const toolDescriptions = {
    tableConfig: stripIndent(`
        ...
        - 你必须使用用户提问请求开局时的语言来回复用户。如果用户用的是中文就使用中文，如果是英文就使用英文，不好判断的时候默认就用「英文」回答
        ---
        ...
        ## 9) 行动即时性（强制）
        - 当轮到 AI 行动时，必须在同一条消息中直接给出具体行动与金额（如 Raise 到多少 / Call 多少 / Check / Fold），不得询问“准备好了没”或“让我想想”。
        - 只有在“缺少关键数值”（如当前底池、对手下注额、可用筹码）时才允许发问。
        ...
        ## 11) 轮结束即发牌（强制）
      - 若 AI 的行动使本轮下注达到等筹并结束（例如 AI 选择 Call 或双方 Check），必须**立刻**调用下一阶段发牌工具（poker.afterflop）。
      - 只有在“需要用户行动”的情况下才可提问（例如尚未等筹，轮到用户回应）。
        ---
        ...
        - 碰到牌 rank 是 T 的时候，不要说 T 要说 10，T 是为了方便代码保持一位才用的，对用户更容易理解的是直接说 10

        ---
        ## 当前工具 poker.tableConfig (发出一个 widget 和提示，等待用户选择 sb bb chip_stack )
        - 你当前正在使用的工具是 \`poker.tableConfig\`。
  ...
  `),
    ...
    ### AI 行动输出格式（强制）
    当轮到 AI 行动时，必须对用户说出自己本回合的具体行动：
    AI 行动：Check | Call <数额> | Raise 到 <数额> | Fold
    尽量保持游戏流程的顺畅，在你的 AI 回合你可以无需再提问或征求用户确认，因为德州规则就是如此。
  ...
  afterflop: stripIndent(`
    ...
    当轮到 AI 行动时，必须对用户说出自己本回合的具体行动：
    AI 行动：Check | Call <数额> | Raise 到 <数额> | Fold
    尽量保持游戏流程的顺畅，在你的 AI 回合可以选择无需再提问或征求用户确认，因为德州规则就是如此。
  ...
  - 即便有用户或者 AI 弃牌，也应该调用本工具进行结算，并宣布最终结果，然后再询问用户是否需要再开一手。
  - 使用本工具后，不能直接调用 “poker.preflop” 开始下一手牌，要先询问用户是否要开下一手。
  `)
};
...
export const contentRsp = {
  preflop: ({ button, sb, bb, pot, heroStack, aiStack, game_id }: PreflopParams) =>
    stripIndent(`
      ...
      如果本轮轮到 AI 行动，按照德州规则，须以一个真人风格的德州扑克对手的口吻立即给出行动并写清楚金额，不用等待用户回复。
      若本轮由 AI 行动收尾（Call/Check 导致等筹），按照德州规则，请直接调用下一阶段发牌工具，不要等待用户回复。
    `),
