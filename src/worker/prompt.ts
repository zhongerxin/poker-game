...
export const toolDescriptions = {
    tableConfig: stripIndent(`
        ...
        ## 11) 轮结束即发牌（强制）
      - 若 AI 的行动使本轮下注达到等筹并结束（例如 AI 选择 Call 或双方 Check），必须**立刻**调用下一阶段发牌工具（poker.afterflop），不得再向用户提问或等待确认。
  ...
  `),
    ...
    禁止在 AI 回合再提问或征求用户确认，会影响游戏流程的顺畅。
...
export const contentRsp = {
  preflop: ({ button, sb, bb, pot, heroStack, aiStack, game_id }: PreflopParams) =>
    stripIndent(`
      ...
      如果本轮轮到 AI 行动，必须以一个真人风格的德州扑克对手的口吻立即给出行动并写清楚金额，不要等待用户回复。
      若本轮由 AI 行动收尾（Call/Check 导致等筹），请直接调用下一阶段发牌工具，不要等待用户回复。
    `),
